...
setwd("/Volumes/kschaumberg/MAXED/MAXED-Data-Cleaning/data")
uri <- 'https://redcap.ictr.wisc.edu/api/'
token <- 'XXXXX' # add redcap API TOKEN HERE
...
rawfilename = paste('MAXED_redcap_raw.', currentDate, '.csv', sep = "")
...
file.copy (from = paste0(old_rdf), to = paste0('old/',old_rdf))
file.remove (from = paste0(old_rdf))

# Save!
rdffilename <-  paste('MAXED_redcap_rdf.', currentDate, '.RData', sep = "")
save(BAMH_redcap_rdf, file = rdffilename)
longfilename <- paste('MAXED_redcap_long.', currentDate, '.RData', sep = "")
save(MAXED_redcap_long, file = longfilename)
widefilename <- paste('MAXED_redcap_wide.', currentDate, '.RData', sep = "")
