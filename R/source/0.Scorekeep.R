...
df <- read.csv(file.path(paste0('data/RedCap/MAXED_redcap_raw.',currentDate,'.csv')))
...
WT_VARS <- df %>% select('record_id', 'redcap_event_name', starts_with(c('edhistory', 'dcf', 'bmi', 'ed100k'))) %>% 
  mutate (dcf_high_wt = as.numeric(dcf_height)) %>% 
  mutate (wt_suppress_high_current_obj_dcf = dcf_high_wt - bmi_weight_kg*2.205) %>% 
  mutate (wt_suppress_high_low_dcf  = dcf_high_wt - as.numeric(dcf_weight_low)) %>% 
  mutate (wt_suppress_high_current_obj_ed100k = as.numeric(ed100k_wt_hi_lb) - bmi_weight_kg*2.205) %>%
  mutate (wt_suppress_high_low_ed100k = as.numeric(ed100k_wt_hi_lb) - as.numeric(ed100k_wt_low)) %>%
  mutate (wt_suppress_high_current = ifelse (!is.na(wt_suppress_high_current_obj_dcf), wt_suppress_high_current_obj_dcf, wt_suppress_high_current_obj_ed100k)) %>%
  mutate (wt_suppress_low = ifelse (!is.na(wt_suppress_high_low_dcf), wt_suppress_high_low_dcf, wt_suppress_high_low_ed100k)) |> 
  select (record_id, redcap_event_name, starts_with('bmi'), wt_suppress_high_current, wt_suppress_low) %>%
